...
export async function generateAudio(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // --- 阅后即焚逻辑 ---
    const fileName = path.basename(result.audio)
    ...
    try {
      ...
      logger.info(`🔥 Burned (deleted) audio file: ${fileName}`)
    } catch (e) {
      logger.error(`Failed to burn audio file: ${e}`)
      // 如果读取失败，可能需要降级处理或报错，这里暂且继续，前端会处理缺失
    }
    ...
    const responseResult = {
      ...
      data: {
        ...result,
        audio_base64: audioBase64, // 返回 Base64
        file: fileName,
        srt: path.parse(result.srt).base,
      },
      code: 200,

...
class EdgeTTS {
  ...
  async ttsPromise(
    text: string,
    options: TtsOptions = {}
  ): Promise<Readable | Buffer | void | { audio: Buffer; metadata: SubLine[] } | { file: string; metadata: SubLine[] }> {
          ...
          try {
            ...
            metadata['Metadata'].forEach((element: any) => {
              subFile.push({
                ...
                charOffset: element['Data']['text']['Offset'],
              })
            })
          } catch {
            // 忽略解析错误
    ...
    if (outputType === 'stream') {
      ...
      return new Promise<{ audio: Buffer; metadata: SubLine[] }>((resolve, reject) => {
        // 修改 resolveBuffer 签名以匹配新返回类型
        resolveBuffer = (audio) => resolve({ audio, metadata: subFile })
        rejectBuffer = reject
      })
    } else if (outputType === 'file') {
      return new Promise<{ file: string; metadata: SubLine[] }>((resolve, reject) => {
        resolveFile = () => resolve({ file: audioPath!, metadata: subFile })
        rejectFile = reject
      })
    } else {

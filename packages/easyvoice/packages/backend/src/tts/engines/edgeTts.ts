...
export class EdgeTtsEngine implements TTSEngine {
  ...
  async synthesize(
    text: string,
    options: TtsOptions
  ): Promise<Buffer | Readable | { audio: Buffer | Readable; metadata: any[] }> {
    const {
      speed = 1.0,
    ...
    const result = await tts.ttsPromise(text, {
      outputType: finaleType as any,
      audioPath: output,
    })

    if (finaleType === 'stream') {
      return result as Readable
    } else {
      // buffer 模式，result 是 { audio: Buffer, metadata: SubLine[] }
      return result as { audio: Buffer; metadata: any[] }
    }
  }
